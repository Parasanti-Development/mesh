...
func (router *Router) listenTCP() {
	...
	go func() {
		...
		for {
			...
			if !ok {
				router.logger.Printf("Error casting remote address %s to TCPAddr", tcpConn.RemoteAddr().String())
				tcpConn.Close()
				continue
			}
			if remoteAddr.Port != router.Port {
				router.logger.Printf("Ignoring connection %s, expected port:%d", tcpConn.RemoteAddr().String(), router.Port)
				tcpConn.Close()
				continue
			}
